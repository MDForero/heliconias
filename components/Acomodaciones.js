        ...
        <article className='flex flex-wrap mx-auto py-8 justify-evenly md:max-w-screen-2xl w-full border-b-2'>
            <section className='md:w-2/6 w-[95%] place-self-center shadow rounded-2xl overflow-hidden p-2   '>
                <figure className='pb-8 '>
                    <Image loading='lazy'src={img.img} alt={img.alt} width={0} height={0} className="w-full mx-auto h-64 object-contain border-orange-500 border-2 rounded-md shadow-2xl shadow-orange-700/40 " />
                </figure>
                <div className=' flex flex-row gap-2'>
                    {acomodaciones.acomodacion.map((item, index) => (<button className={img.img === item.img ? 'w-1/5 h-16 hover:blur-sm ring ring-black rounded-sm overflow-hidden': "w-1/5 h-16 hover:blur-sm ring rounded-sm overflow-hidden"} key={index} onClick={() => setImg(item)}><Image loading='lazy'className='w-full h-full object-cover' src={item.img} alt={item.alt} width={0} height={0} /> </button>))}
                </div>
            </section>
